...
// TEMP: clear OPcache on every page load — remove once confirmed working
//if ( function_exists( 'opcache_reset' ) ) opcache_reset();echo '<!-- OPcache reset at ' . date( 'Y-m-d H:i:s' ) . ' -->';
...
$sc_our = [
    'name'     => 'The New American Freedom Index',
...
.sc-compare-nav .sc-page-label {
    font-size: .8rem;
    color: var(--bs-secondary, #6c757d);
    white-space: nowrap;
}
.sc-compare-nav .sc-col-names {
    font-size: .85rem;
    font-weight: 600;
    flex: 1;
    text-align: center;
...
/* Table wrapper */
.sc-compare-wrap {
    overflow-x: hidden; /* carousel handles visibility; no horizontal scroll */
}
...
.sc-th-logo { height: 75px; }

/* Hide/show carousel columns via JS */
table.sc-compare-tbl .sc-comp-hidden {
    display: none;
}
...
@media (max-width: 767px) {
    ...
    table.sc-compare-tbl .sc-col-label { width: 38%; min-width: 110px; }
    table.sc-compare-tbl .sc-col-our  { width: 34%; min-width: 110px; }
    table.sc-compare-tbl .sc-col-comp { width: 28%; min-width: 100px; }
}
</style>
...
<div id="<?php echo $uid; ?>">
    ...
    <!-- Navigation -->
    <div class="sc-compare-nav">
        <button class="btn btn-sm btn-outline-secondary sc-prev" aria-label="Previous scorecard" disabled>
            <i class="bi bi-chevron-left"></i>
        </button>
        <div class="flex-fill text-center">
            <span class="sc-col-names"><!-- updated by JS --></span>
            <div class="sc-page-label"><!-- updated by JS --></div>
        </div>
        <button class="btn btn-sm btn-outline-secondary sc-next" aria-label="Next scorecard">
    ...
    <table class="sc-compare-tbl">
        <thead>
            <tr>
                ...
                <th class="bg-white align-bottom">
                    <?php if ( ! empty( $sc_our['logo_url'] ) ) : ?>
                        <img src="<?php echo esc_url( $sc_our['logo_url'] ); ?>" alt="<?php echo esc_attr( $sc_our['name'] ); ?>" class="img-fluid">
                    <?php endif; ?>
                </th>
                <?php foreach ( $sc_compare as $ci => $col ) : ?>
                <th class="bg-white align-bottom sc-th-logo" data-comp-col="<?php echo $ci; ?>" data-sc-name="<?php echo esc_attr( $col['name'] . ( ! empty( $col['sub'] ) ? ' / ' . $col['sub'] : '' ) ); ?>">
			...
			<tr>
                ...
                <th class="sc-col-our">
                    <?php echo esc_html( $sc_our['name'] ); ?>
                    <?php if ( ! empty( $sc_our['sub'] ) ) : ?>
                        <span class="sc-th-sub"><?php echo esc_html( $sc_our['sub'] ); ?></span>
                    <?php endif; ?>
                </th>
                <?php foreach ( $sc_compare as $ci => $col ) : ?>
                <th class="sc-col-comp" data-comp-col="<?php echo $ci; ?>" data-sc-name="<?php echo esc_attr( $col['name'] . ( ! empty( $col['sub'] ) ? ' / ' . $col['sub'] : '' ) ); ?>">
        ...
        <tbody>
            ...
            <tr>
                ...
                <?php foreach ( $row as $ci => $cval ) : ?>
                <td class="sc-col-comp" data-comp-col="<?php echo $ci; ?>"><?php echo $sc_cell( $cval ); ?></td>
                <?php endforeach; ?>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
(function () {
    ...
    var prevBtn   = wrap.querySelector('.sc-prev');
    var nextBtn   = wrap.querySelector('.sc-next');
    var colLabel  = wrap.querySelector('.sc-col-names');
    var pageLabel = wrap.querySelector('.sc-page-label');
    var allCompCells = wrap.querySelectorAll('[data-comp-col]');

    var TOTAL     = <?php echo $num_comp; ?>;
    var page      = 0;

    // Build header name lookup: comp-col index → display name
    var colNames = [];
    var headerCells = wrap.querySelectorAll('thead [data-comp-col]');
    headerCells.forEach(function(th) {
        var idx = parseInt(th.getAttribute('data-comp-col'), 10);
        colNames[idx] = th.getAttribute('data-sc-name') || '';
    });
    ...
    function render() {
        ...
        allCompCells.forEach(function(el) {
            var idx = parseInt(el.getAttribute('data-comp-col'), 10);
            el.classList.toggle('sc-comp-hidden', idx < start || idx >= end);
        });
        ...
        // Update label: show names of visible scorecards
        var names = [];
        for (var i = start; i < end; i++) {
            if (colNames[i]) names.push(colNames[i]);
        }
        colLabel.textContent  = names.join(' · ');
        pageLabel.textContent = 'Comparing ' + (page + 1) + ' of ' + totalPages();
    }
    ...
    // Re-render on resize (perPage may change)
    var resizeTimer;
    window.addEventListener('resize', function() {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(function() {
            // Clamp page index to new total
