...
foreach ($legislator_data as $data) {
    ...
    if ($party_name !== '') {
		//Exclude null N/A from average calculation
		if($score !== null) {
			$party_count[$party_name] = ($party_count[$party_name] ?? 0) + 1;
			$party_sum[$party_name] = ($party_sum[$party_name] ?? 0) + $score;
		}
    }
}
    ...
    <table class="table table-bordered table-sm table-hover">
        <thead class="table-dark">
            <tr>
				...
				foreach ($votes as $vote){
                    echo '<th scope="col" class="text-center">'.($vote_start + $v).'</th>';
					$v++;
                } ?>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($legislator_data as $data):
                ...
                $leg_url = fi_legislator_get_url($leg->id ?? 0, $gov);
