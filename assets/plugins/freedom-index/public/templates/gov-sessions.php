					...
					<div class="swiper-slide">
						<div class="card h-100 border-0 rounded-bottom-4" style="min-height:<?php echo esc_attr($height); ?>; margin: 0 36px;">
							...
							<?php foreach ($session_reports_list as $report): ?>
								<a href="<?php echo esc_url(fi_report_get_url($report['id'] ?? 0, $gov_slug)); ?>" class="btn btn-sm btn-outline-primary fw-bold w-100 mb-2 fi-nav-item fs-7">
									<?php echo esc_html($report['title_menu'] ?: ($report['title'] ?: 'Untitled Report')); ?>
								</a>
							<?php endforeach; ?>
							</div>
						</div>
...
document.addEventListener('DOMContentLoaded', function() {
	if (typeof Swiper !== 'undefined') {
		new Swiper('.fi-sessions-swiper', {
			...
			spaceBetween: 0,
			navigation: {
				nextEl: '.fi-sessions-swiper .swiper-button-next',
				prevEl: '.fi-sessions-swiper .swiper-button-prev',
			},
			// Disable auto-scroll
			autoplay: false,
			// Enable touch/swipe
