...
//fi_log('party_chamber_data: ' . json_encode($party_chamber_data),__FILE__,__LINE__);	
		...
		public static function render($args = array()): string {
			$defaults = array(
				...
				'htmx' => true, // Enable HTMX for refresh-less filtering
...
if (strtoupper($gov) === 'US'){
	...
	$col_name = '3';
			...
			if (!empty($args['htmx']) && $is_legislators_page) {
				$htmx_target = esc_attr($args['htmx_target'] ?? '#fi-legislators-results');
				$htmx_indicator = esc_attr($args['htmx_indicator'] ?? '.fi-htmx-indicator');
				$htmx_attrs = sprintf(
					...
					$htmx_target,
					$htmx_indicator,
					'#fi-legislators-grid, #fi-legislators-results > *'
				);
			}
			?>
...
<!--						<label for="fi-state" class="<?= $classFormLabel; ?>">State</label> -->
					...
					<!-- Submit Button (legacy AJAX only) -->
			...
			jQuery(document).ready(function($) {
				...
				// HTMX pretty URL building
				if (htmxEnabled && typeof htmx !== 'undefined') {
					// Listen for HTMX requests and build pretty URLs
					filterForm.on('htmx:configRequest', function(evt) {
						...
						// Override the URL HTMX will use
						evt.detail.path = url;
						
						// Clear parameters (they're in the URL path now)
						evt.detail.parameters = {};
					});
					
					// Skip old AJAX behavior - HTMX handles everything
					return;
				}
