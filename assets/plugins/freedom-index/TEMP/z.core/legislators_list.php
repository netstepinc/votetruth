 ...
 * }
 * @return array Array of legislator objects for card display
 */
...
/**
 * Format raw row into card-ready object
 */
function legislators_list_format_legislator(object $row): object {
    $leg = new stdClass();
    
    $leg->id = (int) $row->id;
    $leg->gov = $row->gov;
    $leg->display_name = $row->display_name ?: trim($row->first_name . ' ' . $row->last_name);
    $leg->first_name = $row->first_name;
    $leg->last_name = $row->last_name;
    $leg->image_id = $row->image_id ? (int) $row->image_id : null;
    $leg->image_url = $row->image_url ?: $row->legacy_image_url ?: null;
    $leg->score = $row->score !== null ? (int) $row->score : null;
    $leg->party = $row->party;
    $leg->chamber = $row->chamber;
    $leg->state = $row->state;
    $leg->district = $row->district;
    $leg->district_name = $row->district_name;
    $leg->session_id = (int) $row->session_id;
    $leg->session_name = $row->session_name;
    $leg->session_parent_id = $row->session_parent_id ? (int) $row->session_parent_id : null;
