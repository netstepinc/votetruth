...
function fi_public_inline_js() {
	...
	?>
<script>
(function($){'use strict';window.FI={ajaxurl:'<?= $ajax_url ?>',nonce:'<?= $nonce ?>',isLoggedIn:<?= $is_logged ?>,currentGov:'',currentSession:'',currentUserId:<?= $user_id ?>};$(document).ready(function(){FI.init();setTimeout(function(){FI.initFrontPageSearch();},0);});FI.init=function(){
        FI.initSearch();
        FI.initLists();
        FI.initUserPrefs();
        FI.initAnimations();
