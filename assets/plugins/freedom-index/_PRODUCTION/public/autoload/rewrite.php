...
//			$legacy = get_query_var('fi_legacy');
			...
			if (strpos($request_uri, '/legislators') !== false || $debug_rewrite) {
				//self::log('REWRITE DEBUG: REQUEST_URI=' . $request_uri . ' | gov=' . ($gov ?: 'EMPTY') . ' | entity=' . ($entity ?: 'EMPTY') . ' | slug=' . ($slug ?: 'EMPTY'));
			}
...
*/
		...
		* Formats: list,cards
		* https://freedomindex.us/?fi_gov=fl&fi_entity=legislators&fi_session=85&fi_pdf=cards
		*/
		private function handle_legislators_list_request(string $gov): void {
			...
			// Don't load legislators here - they're loaded via AJAX
			$legislators = []; // Empty array - loaded via AJAX

			// Debug output (remove after testing)
			self::log('FI Rewrite Debug - gov: ' . $gov . ', current_session: ' . ($current_session ?: 'null') . ' | parsed_filters: ' . json_encode($parsed_filters) . ' | filter_args: ' . json_encode($filter_args), __FILE__, __LINE__, 'debug');
			
			// Get session ID from filter args (for reports and filter description)
			$session_id = $filter_args['session'] ?? $current_session;
			...
			// Set up global variables for template
			// NOTE: Don't load legislators server-side - they'll be loaded via AJAX on first filter interaction
			global $fi_legislators, $fi_gov, $fi_gov_name, $fi_session, $fi_reports, $fi_report_links, $fi_filter_description;
			$fi_legislators = []; // Empty array - will be loaded via AJAX
			$fi_gov = $gov;
			$fi_gov_name = fi_gov_name($gov);
		...
		*/
		private function handle_legislator_request(?string $gov, ?int $legislator_id = null): void {
			...
			if (!function_exists('fi_legislator_get')) {
				$this->handle_404();
				return;
			}
			...
			// Load template
			//Testing new legislator template
			if(isset($_GET['TEST']) && $_GET['TEST'] == 'leg_old' ){
				$this->load_template('legislator');
			}else{
				$this->load_template('legislator-api');
			}
		}
