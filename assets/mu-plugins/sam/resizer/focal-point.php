...
if( ! class_exists('JBS_image_sizes_focal_point') ){
    class JBS_image_sizes_focal_point{
            ...
            if( in_array( get_post_mime_type( $post->ID ), SIS_ALLOWED_MIME_TYPES ) ){
                ...
                <div style="display:flex;align-items:center;gap:40px">
                    <div class="focal-point-values">
                        <b><?php esc_html_e( 'top', 'jbs-image-sizes' ) ?></b>: <span class="focal-point-top" data-value="<?php echo floatval( $focal_point[1] ) ?>"><?php echo round( $focal_point[1] * 100 ) ?></span>%
                        <br>
                        <b><?php esc_html_e( 'left', 'jbs-image-sizes' ) ?></b>: <span class="focal-point-left" data-value="<?php echo floatval( $focal_point[0] ) ?>"><?php echo round( $focal_point[0] * 100 ) ?></span>%
                    </div>
                    <button type="button" class="button button-small pick-focal-point">
                        <?php esc_html_e( 'Pick focal point', 'jbs-image-sizes' ) ?>
                    </button>
                    <button type="button" class="button button-small save-focal-point" style="display:none">
                        <?php esc_html_e( 'Save focal point', 'jbs-image-sizes' ) ?>
                    </button>
                </div>
                ...
                $form_fields['focal_point'] = array(
                    ...
                    'label' => __( 'Focal point', 'jbs-image-sizes' ),
                    'input' => 'html',
                    'html' => $html
                );
            }
        ...
        function admin_enqueue_scripts(){
            wp_enqueue_style( 'image-sizes-focal-point', SIS_RESIZER_URL . '/focal-point-style.css' );
            wp_enqueue_script( 'image-sizes-focal-point', SIS_RESIZER_URL . '/focal-point-script.js' );
        }
    }

    new JBS_image_sizes_focal_point();
}
